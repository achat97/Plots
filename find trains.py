...
def search_slot(data, fs, replicas, t_pred, search, min_snr):

    """
    Looks for a ping at one predicted time, trying every waveform the train is known to use.

    A missing ping's type is not known in advance, so each replica is searched and the best score
    decides which waveform was there. When a waveform's band also moves from ping to ping, the
    centre frequency is scanned as well, with a step suited to the pulse: a CW tone occupies a
    single frequency bin, so a step chosen for a wide sweep would walk straight past it.
    ...
    Parameters:
        ...
        replicas (list) - the train's waveforms, from train_replicas.
        t_pred (float) - predicted pulse start in seconds.
        search (float) - half-width of the window searched around it, in seconds.
        min_snr (float) - dB above the surrounding noise required to accept a ping.

    Returns:
        found (dict or None) - the accepted ping, or None when nothing cleared the threshold.
        best (tuple) - (score, t_found, replica) of the best attempt, accepted or not, so the
                       caller can report what the strongest match was.
        alt (str) - a note naming the best score from a different waveform, or an empty string.
    """
    ...
    for rep in replicas:
        if rep["spread"] > 400.0:
            ...
            step = max(2.0 * fs / NPERSEG, 0.25 * bw, rep["spread"] / 40.0)
            shifts = np.arange(-rep["spread"] / 2, rep["spread"] / 2 + step, step)
        else:
            shifts = [0.0]
        for sh in shifts:
            score, t_found = matched_filter(data, fs, rep["wave"], t_pred, search,
                                            rep["f1"] + sh, rep["f2"] + sh,
                                            duration=rep["duration"], kind=rep["type"])
            tried.append((score, t_found, {**rep, "f1": rep["f1"] + sh, "f2": rep["f2"] + sh}))
    ...
    if others:
        ...
        alt = f", best {type_name(best_other[2]['type'])} scored {best_other[0]:+.1f} dB"

    if score < min_snr:
        return None, (score, t_found, rep), alt
    return ({"t_start": t_found, "t_stop": t_found + rep["duration"],
             "f1": rep["f1"], "f2": rep["f2"], "type": rep["type"],
             "score": score, "duration": rep["duration"], "alt": alt},
            (score, t_found, rep), alt)


def extend_train(data, fs, train, replicas, search, min_snr, duration, tol,
                 max_gap_slots, all_detections, direction, max_slots=200):

    """
    Follows a train outward past its last detected ping, in one direction, for as long as pings
    keep being found.

    A train's first and last detections are usually not where the transmission started and
    stopped: they are where the detector stopped coping. Once an interval has been established
    from many pings, the slot immediately beyond the last one is as firm a prediction as any gap
    inside the train, and it is checked the same way. Each ping that is found becomes evidence in
    its own right, so the walk continues from there - a train that runs for forty more slots is
    followed for forty more slots.

    Two things keep this from running away. The walk stops after max_gap_slots consecutive slots
    come back empty, which is the same rule that ends a train internally. And the interval is
    re-fitted whenever a ping is recovered, so the predicted times stay anchored to the pings
    actually seen instead of extrapolating an interval that was estimated once and never revised;
    without that, the prediction drifts by its own error a little more with every slot, and far
    enough out the search would be scanning rather than predicting.
    ...
    Parameters:
        ...
        train (dict) - one train from build_trains.
        replicas (list) - the train's waveforms, from train_replicas.
        search (float) - half-width of the window searched around a predicted time.
        min_snr (float) - dB above the surrounding noise required to accept a ping.
        duration (float) - length of the recording in seconds.
        tol (float) - relative tolerance, used to decide whether a slot is already occupied.
        max_gap_slots (int) - consecutive empty slots that end the walk.
        all_detections (list) - every detection, so a slot already holding one is not searched.
        direction (int) - +1 to walk forward from the last member, -1 to walk back from the first.
        max_slots (int) - default 200. Hard bound on how far the walk may run, so that a train
                          cannot be followed across a whole recording however well each step
                          scores. The stopping rule normally ends the walk long before this.

    Returns:
        found (list) - the recovered pings, each with the slot number it was found at.
    """

    members = train["members"]
    period = train["interval"]
    anchor = members[-1]["t_start"] if direction > 0 else members[0]["t_start"]
    have = np.array([d["t_start"] for d in all_detections]) if all_detections else np.array([])

    # times of the pings supporting the fit, used to re-estimate the interval as the walk proceeds
    times = [d["t_start"] for d in members]

    found, misses, slot, steps = [], 0, 0, 0
    while misses <= max_gap_slots and steps < max_slots:
        slot += 1
        steps += 1
        t_pred = anchor + direction * slot * period
        if t_pred < 0.0 or t_pred > duration:
            break

        # a slot already holding a detection is not a gap, and is not a miss either
        if have.size and np.min(np.abs(have - t_pred)) < tol * period:
            misses = 0
            continue

        ping, best, _alt = search_slot(data, fs, replicas, t_pred, search, min_snr)
        if ping is None:
            misses += 1
            continue

        misses = 0
        ping["t_predicted"] = t_pred
        ping["slot"] = direction * slot
        found.append(ping)

        # Re-anchor: the recovered ping is evidence, so the interval is re-fitted to everything
        # seen so far and the walk continues from the ping itself.
        times.append(ping["t_start"])
        ordered = np.array(sorted(times))
        k = np.round((ordered - ordered[0]) / period)
        if k.max() > 0:
            A = np.vstack([np.ones_like(k), k]).T
            try:
                coef, *_ = np.linalg.lstsq(A, ordered, rcond=None)
                if coef[1] > 0:
                    period = float(coef[1])
            except np.linalg.LinAlgError:
                pass
...
def matched_filter(data, fs, replica, t_predicted, search, f1, f2, pad_hz=1000.0,
    ...
    # Background: the SAME track, at the same frequencies, placed at other times.
    #
    # An earlier version displaced the track in frequency instead, and that is wrong wherever the
    # noise spectrum has a slope - which it always does in the ocean, where levels fall steeply
    # with frequency. A track at 1 kHz compared with copies at 5 and 9 kHz then scores well above
    # the threshold on noise containing no pulse at all, simply because low frequencies are louder:
    # measured at 13 to 14 dB against a 6 dB threshold, so every low-frequency slot looked like a
    # ping and the search reported pulses everywhere. Comparing against other times removes the
    # spectral slope entirely, since both measurements are made in the same band.
    energies = []
    for start_col in range(0, S.shape[1] - n_cols):
        on = track_energy(start_col, 0.0)
        if on is not None:
            energies.append((on, start_col))
    if not energies:
        return 0.0, t_predicted

    levels = np.array([e for e, _c in energies])
    # the median over the crop is the level of this band when nothing is happening in it; a real
    # ping stands above it, and the crop is mostly background by construction
    background = float(np.median(levels))

    best_score, best_col = -np.inf, 0
    for on, start_col in energies:
        score = on - background
        if score > best_score:
...
def plot_trains(trains, loners, recovered, duration, path, fs):
    ...
    """
    Draws the trains in the same two-panel layout as the detection and refinement overviews:
    frequency against time above, and the match score of the recovered pings below.

    Each pulse is a short vertical line spanning its frequency band, coloured by its own type,
    since a train may mix waveforms. Each train is marked by a single horizontal rule at its median
    frequency, running from its first pulse to its last, and labelled with its interval at the left
    end. An earlier version instead joined consecutive pulses with a line through their band
    centres; with mixed waveforms at different frequencies that line zig-zagged across the figure
    and obscured the pulses it was meant to group. A horizontal rule says the same thing - these
    pulses belong together, and they run from here to here - without crossing anything.

    The title is kept to one line. Per-train details live on the rules themselves, so the figure
    does not shrink as more trains are found.
    ...
    seen = set()
    labelled = []          # where labels have been placed, so two trains do not write over each other
    for i, train in enumerate(trains):
        members = train["members"]
        got = [r for r in recovered if r["train"] == i]
        every = members + [{"t_start": r["t_start"], "t_stop": r["t_stop"],
                            "f1": r["f1"], "f2": r["f2"], "type": r["type"]} for r in got]

        # one horizontal rule per train: where it runs, and in which band
        centre = float(np.median([0.5 * (d["f1"] + d["f2"]) for d in every]))
        t_lo = min(d["t_start"] for d in every)
        t_hi = max(d["t_stop"] for d in every)
        ax_f.plot([t_lo, t_hi], [centre, centre], "-", color="0.55", lw=0.9, alpha=0.8,
                  zorder=1)
        # Two trains in the same band start their labels at the same place; each is lifted clear
        # of any label already written near it.
        dy = 9.0
        while any(abs(t_lo - x) < 0.12 * max(duration, 1e-3) and abs(dy - y) < 10.0
                  for x, y in labelled):
            dy += 11.0
        labelled.append((t_lo, dy))
        ax_f.annotate(f"train {i}: {train['interval']:.2f} s", (t_lo, centre),
                      textcoords="offset points", xytext=(2, dy), fontsize=7, color="0.3",
                      bbox=dict(boxstyle="round,pad=0.15", fc="white", ec="none", alpha=0.75))

        # A CW pulse has f1 == f2, so drawn faithfully it is a dot and disappears among the
        # swept pulses. Every pulse is given a minimum drawn height, as the detection boxes are.
        min_h = 0.015 * fs / 2
        ...
        for d in members:
            ...
            colour = TYPE_COLORS.get(d["type"], REFERENCE)
            label = type_name(d["type"]) if d["type"] not in seen else None
            seen.add(d["type"])
            ax_f.plot([mid, mid], [lo, hi], color=colour, lw=2.2, solid_capstyle="round",
        ...
        for r in got:
            ...
            colour = TYPE_COLORS.get(r["type"], REFERENCE)
            ax_f.plot([mid, mid], [lo, hi], color=colour, lw=1.4, ls="--", alpha=0.85, zorder=2)
            ax_f.plot([mid], [0.5 * (lo + hi)], marker="o", ms=5, mfc="none", mew=1.3,
                      color=ACCENT, zorder=4,
                      label="Recovered" if "rec" not in seen else None)
            seen.add("rec")
            ax_s.plot([mid, mid], [0.0, r["score"]], color=ACCENT, lw=1.2)
            ax_s.plot([mid], [r["score"]], marker="o", ms=3, color=ACCENT)

    for d in loners:
        mid = 0.5 * (d["t_start"] + d["t_stop"])
        ax_f.plot([mid], [0.5 * (d["f1"] + d["f2"])], marker="x", ms=5, color="0.6",
                  label="No train" if "lone" not in seen else None, zorder=2)
        seen.add("lone")

    ax_f.set_xlim(0, max(duration, 1e-3))
    ax_f.set_ylim(0, fs / 2)
    n_ext = sum(1 for r in recovered if r.get("where") == "extended")
    extra = f" ({n_ext} beyond a train's ends)" if n_ext else ""
    finish(ax_f, None, "Frequency [Hz]",
           f"{len(trains)} train(s), {len(recovered)} ping(s) recovered{extra}",
           legend=bool(seen))
    ax_s.set_ylim(bottom=0)
    hms_axis(ax_s)
    finish(ax_s, "Time [h:mm:ss]", "Recovery score [dB]", None)
    save(fig, path)
...
def main():
    ...
    ap.add_argument("--max-gap-slots", type=int, default=5,
                    help="consecutive empty slots that end a train, both inside it and when "
                         "following it outward")
    ap.add_argument("--min-snr-extend", type=float, default=10.0,
                    help="dB required to accept a ping BEYOND a train's ends. Held higher than "
                         "--min-snr because a gap inside a train is bounded by confirmed pings on "
                         "both sides, while one past the end has support on one side only, so a "
                         "marginal match has less to contradict it")
    ap.add_argument("--max-extend", type=int, default=200,
                    help="hard bound on how many slots a train may be followed past each end; "
                         "the stopping rule normally ends the walk long before this")
    ap.add_argument("--no-extend", action="store_true",
                    help="only fill gaps inside a train; do not follow it past its first and "
                         "last detected ping")
    ...
    if not args.no_recover:
        for i, train in enumerate(trains):
            # A train with no interior gaps still gets followed outward: having every slot filled
            # between the first and last detection says nothing about whether the transmission
            # continued past them.
            ...
            # gaps inside the train
            for t_pred in gaps:
                ping, best, _alt = search_slot(data, args.fs, replicas, t_pred, search,
                                               args.min_snr)
                if ping is None:
                    score, _tf, rep = best
                    print(f"    gap at {hms(t_pred)} (train {i}): best {score:+.1f} dB "
                          ...
                          f"-> nothing there")
                    continue
                ping.update({"train": i, "t_predicted": t_pred, "where": "interior"})
                candidates.append(ping)

            # and beyond both ends, for as long as pings keep being found
            if not args.no_extend:
                for direction, label in ((-1, "before"), (1, "after")):
                    got = extend_train(data, args.fs, train, replicas, search,
                                       max(args.min_snr, args.min_snr_extend),
                                       duration, args.tol, args.max_gap_slots, dets, direction,
                                       max_slots=args.max_extend)
                    for ping in got:
                        ping.update({"train": i, "where": "extended"})
                        candidates.append(ping)
                    if got:
                        print(f"    train {i}: {len(got)} ping(s) recovered {label} the "
                              f"detected train")
    ...
    for r in recovered:
        ...
        where = r.get("where", "interior")
        print(f"    recovered at {hms(r['t_start'])} ({where}): {type_name(r['type'])} from "
              f"train {r['train']}, {r['score']:+.1f} dB over background"
              f"{r.get('alt', '')}{note}")

    path = args.out or os.path.splitext(args.detections)[0] + "_trains.csv"
    header = ["source", "recovery", "train_id", "pulse_id", "t_start_s", "t_stop_s",
    ...
    with open(path, "w", newline="") as fh:
        ...
        for d in loners:
            ...
            w.writerow(table[-1])
        for r in recovered:
            table.append(["recovered", r.get("where", "interior"), r["train"], "",
                          round(r["t_start"], 3),
