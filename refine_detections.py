...
Refinement is normally only a measurement: it does not add or remove detections. It can, however,
be used as a second opinion on whether a pulse was there at all - a detection with no trackable
ridge behind it usually is not one - which --drop-unrefined turns on. Measured on synthetic data,
that removes every false alarm raised on a broadband transient or on empty noise, and none of the
ones raised on a steady tonal, which does produce a ridge; real pulses above about -12 dB SNR are
unaffected. See measure_refine_filter.py. A pulse whose ridge
...
def refine_spectrogram(S, f_axis, t_axis, det, snr_db=None, rel_db=None, min_points=8,
                       max_rmse_bins=3.0, truncate_echo=True, min_bandwidth=50.0,
                       pad_hz=1500.0):

    """
    Refines one detection from a spectrogram that has already been computed, rather than from the
    recording it came from.

    The refinement normally starts from the waveform, because deployment gives it a recording. A
    stored dataset gives spectrograms instead - the phase is gone, so the waveform cannot be
    recovered - and this is the entry point for that case. It runs the same ridge tracking, sweep
    fitting and classification on the array supplied, so a measurement made here describes the
    refinement as it is actually used.

    ----------

    Parameters:
        S (ndarray) - the spectrogram in dB, shape (frequency bins, time bins).
        f_axis (ndarray) - frequency of each row in Hz.
        t_axis (ndarray) - time of each column in seconds.
        det (dict) - the detection, with keys t_start, t_stop, f1, f2, in the same units as the
                     axes.
        snr_db, rel_db (float or None) - ridge thresholds; None searches them per pulse.
        min_points (int) - default 8. Fewest ridge points accepted.
        max_rmse_bins (float) - default 3.0. Largest accepted fit residual, in frequency bins.
        truncate_echo (bool) - default True. Cut the ridge where an echo takes over.
        min_bandwidth (float) - default 50.0. Below this the pulse is reported as CW.
        pad_hz (float) - default 1500.0. Width added to each side of the predicted band.

    Returns:
        out (dict) - as refine_one: 'ok' False with a 'reason' when the pulse could not be
                     measured.
    """
    ...
    lo = max(float(f_axis[0]), min(det["f1"], det["f2"]) - pad_hz)
    hi = min(float(f_axis[-1]), max(det["f1"], det["f2"]) + pad_hz)
    if hi - lo < 4 * df:
        lo, hi = max(float(f_axis[0]), lo - pad_hz), min(float(f_axis[-1]), hi + pad_hz)

    band = (f_axis >= lo) & (f_axis <= hi)
    if band.sum() < 3:
        return {"ok": False, "reason": "search band outside the spectrogram"}

    duration = max(det["t_stop"] - det["t_start"], 1e-3)
    max_step = max(8 * df, 3.0 * (hi - lo) / duration * dt)
    ...
    best = None
    for s_thr in snr_grid:
        for r_thr in rel_grid:
            t, f, level = _ridge_from_array(S, f_axis, t_axis, band, s_thr, r_thr,
                                            max_step, df)
            if len(t) < min_points:
                continue
            inside = (t >= det["t_start"]) & (t <= det["t_stop"])
            if inside.sum() >= min_points:
                t, f, level = t[inside], f[inside], level[inside]
            if truncate_echo:
                t, f, level, _cut = truncate_at_echo(t, f, level, df, min_points=min_points)
            if len(t) < min_points:
                continue
            fits = fit_sweeps(t, f, float(t[0]), float(t[-1]))
            kind, fit, margin = classify(fits, len(t), min_bandwidth=min_bandwidth)
            if fit is None:
                continue
            if best is None or (fit["rmse"], -len(t)) < (best[0]["rmse"], -best[1]):
                best = (fit, len(t), kind, margin, float(t[0]), float(t[-1]))

    if best is None:
        return {"ok": False, "reason": "no ridge long enough at any threshold", "n_points": 0}
    fit, n_pts, kind, margin, t0, t1 = best
    if fit["rmse"] > max_rmse_bins * df:
        return {"ok": False, "reason": f"ridge scatter {fit['rmse']:.0f} Hz too large",
                "n_points": n_pts}

    f1, f2 = fit["f1"], fit["f2"]
    if kind == "cw":
        f1 = f2 = 0.5 * (f1 + f2)
    return {"ok": True, "type": kind, "f1": f1, "f2": f2, "t_start": t0, "t_stop": t1,
            "rmse": fit["rmse"], "n_points": n_pts, "margin": margin}


def _ridge_from_array(S, f_axis, t_axis, band, snr_db, rel_db, max_step_hz, df):

    """
    The ridge tracker of track_ridge, applied to a spectrogram that is already in hand. See
    track_ridge for what each step is doing and why.

    ----------

    Parameters:
        S (ndarray) - spectrogram in dB.
        f_axis, t_axis (ndarray) - the axes.
        band (ndarray) - boolean mask selecting the search band.
        snr_db (float) - required excess over the median level of a time bin.
        rel_db (float) - level range kept below the strongest column.
        max_step_hz (float) - half-width of the band kept around the track's centre.
        df (float) - frequency bin width in Hz.

    Returns:
        t, f, level (ndarray) - the tracked points.
    """
    ...
    keep = level >= level.max() - rel_db
    t, f, level = t[keep], f[keep], level[keep]
    if len(t) and max_step_hz is not None:
        anchor = float(np.median(f))
        span = max_step_hz * max(len(t), 1) * 0.5
        keep = np.abs(f - anchor) <= max(span, max_step_hz)
        t, f, level = t[keep], f[keep], level[keep]
...
def main():
    ...
    ap.add_argument("--drop-unrefined", action="store_true",
                    help="omit detections that could not be refined, instead of keeping the "
                         "network's values for them. A detection with no trackable ridge behind "
                         "it is usually not a pulse, so this acts as a second opinion on whether "
                         "one was there; measure the cost in recall before relying on it")
    ...
    out_rows, results, n_ok, n_plots, n_dropped = [], [], 0, 0, 0
    ...
    for i, row in enumerate(rows):
        ...
        if args.drop_unrefined and not res["ok"]:
            n_dropped += 1
            continue
        out_rows.append(out)
    ...
    if not out_rows:
        print("\nNothing to write: the detections file was empty, or every detection was "
              "dropped by --drop-unrefined.")
        return
    ...
    if args.drop_unrefined:
        print(f"\nRefined {n_ok} of {len(rows)} detections; {n_dropped} that could not be "
              f"refined were DROPPED (--drop-unrefined).")
    else:
