...
| `--min-snr-extend` | 10.0 | dB required beyond a train's ends, held higher than `--min-snr` because such a match has support on one side only |
| `--max-extend` | 200 | hard bound on slots followed past each end |
...
**The background is estimated at other TIMES, not other frequencies.** The match score compares
the energy on the expected sweep with the same track elsewhere. Displacing it in *frequency* is
wrong wherever the noise spectrum slopes — which it always does in the ocean — because a
low-frequency track then beats a high-frequency reference whether or not a pulse is there.
Measured on noise with a realistic slope and no pulses at all, a 1 kHz track scored **13.8 dB**
against a 6 dB threshold, so every low-frequency slot looked like a ping. Comparing against other
times in the same band removes the slope entirely: the same test now scores 1.9 dB, while a real
pulse there scores 25 dB.
