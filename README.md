...
| `--min-pulses` | 3 | fewest pulses on a consistent interval to call a group a train |
...
the interval. A candidate is accepted only if the arrivals are regular in two senses at once: they
sit tightly on the grid (circular phase spread below `--max-scatter`) **and** they fill at least
`--min-occupancy` of the slots it implies. Tightness alone is not enough, since a sparse grid can
